<script lang="ts">
    ...
    import { likeLoading } from '$lib/store';
    ...
    import Reply from '../../../../components/sections/blog/devlog/reply.svelte';

    export let data;

    $: ({ supabase, comments, session } = data);
    ...
    const addReply = async (event: any) => {
        const reply = event.detail.reply;
        const comment_id = event.detail.comment_id;

        const clean_reply = DOMPurify.sanitize(reply, { FORBID_TAGS: ['a'] });
        if (reply) {
            let { data: _reply, error } = await supabase
                .from('comments')
                .insert([
                    {
                        comment: clean_reply,
                ...
                .select(`id,comment,likes,created_at,accounts(email),liked_by`)
                .single();

            if (!error) {
                let commentIndex = comments?.findIndex((_comment) => _comment.id === comment_id);
                comments?.at(commentIndex!)?.comments.push(_reply);
                comments = comments;
            }
        }
    };
    ...
    const addComment = async (event: any) => {
        ...
        const clean_comment = DOMPurify.sanitize(comment, { FORBID_TAGS: ['a'] });

        if (comment) {
            let { data: _comment, error } = await supabase
                .from('comments')
                .insert([
                    {
    ...
    const updateLikes = async (event: any) => {
        const comment_id = event.detail.comment_id;
        const reply_id = event.detail.reply_id;

        let commentIndex = comments?.findIndex((_comment) => _comment.id === comment_id);
        let replyIndex;
        let rpc_function = 'increment_likes';

        if (reply_id != undefined) {
            replyIndex = comments
                ?.at(commentIndex!)
                ?.comments.findIndex((reply) => reply.id === reply_id);

            if (
                comments![commentIndex!].comments[replyIndex!].liked_by &&
                comments![commentIndex!].comments[replyIndex!].liked_by.includes(session?.user.id)
            ) {
                rpc_function = 'decrement_likes';
            }
        } else {
            if (
                comments![commentIndex!].liked_by &&
                comments![commentIndex!].liked_by.includes(session?.user.id)
            ) {
                rpc_function = 'decrement_likes';
            }
        }
        ...
        if (comment_id) {
            let { data: _comment, error } = await supabase.rpc(rpc_function, {
                row_id: reply_id || comment_id,
                uid: session?.user.id
            });

            if (!error) {
                if (replyIndex != undefined) {
                    if (rpc_function === 'decrement_likes') {
                        let i = comments![commentIndex!].comments[replyIndex].liked_by.findIndex(
                            (uid: string | undefined) => uid === session?.user.id
                        );
                        comments![commentIndex!].comments[replyIndex].liked_by.splice(i, 1);
                        comments![commentIndex!].comments[replyIndex].likes -= 1;
                    } else {
                        if (comments![commentIndex!].comments[replyIndex].liked_by) {
                            comments![commentIndex!].comments[replyIndex].liked_by.push(
                                session?.user.id
                            );
                        } else {
                            comments![commentIndex!].comments[replyIndex].liked_by = [
                    ...
                    if (rpc_function === 'decrement_likes') {
                        let i = comments![commentIndex!].liked_by.findIndex(
                            (uid: string | undefined) => uid === session?.user.id
                        );
                        comments![commentIndex!].liked_by.splice(i, 1);
                        comments![commentIndex!].likes -= 1;
                    } else {
                        if (comments![commentIndex!].liked_by) {
                            comments![commentIndex!].liked_by.push(session?.user.id);
                        } else {
                            comments![commentIndex!].liked_by = [session?.user.id];
                        }
                        comments![commentIndex!].likes += 1;
                    }
                }
...
<section class="bg-foreground text-card/50">
    <h1
    ...
    <div class="bg-background py-12">
        <div class="container">
            <div class="flex flex-col">
                <p class="text-card">
                    <span class="uppercase text-primary-blue">{doc.category}</span> |
                    <span class="text-card/50">Posted by {doc.author}</span>
                </p>
                <p>
                    {date ? date?.getDate() : ''}
                ...
                <!-- <p>{doc.date.getDate()}</p> -->
            </div>
            <h1 class="text-h_2 text-card py-10 2xl:w-4/5">{doc.title}</h1>
            <!-- svelte-ignore a11y-no-static-element-interactions -->
            <div class="relative mb-16 w-max" on:mouseenter={toggleNav} on:mouseleave={toggleNav}>
                <!-- svelte-ignore a11y-mouse-events-have-key-events -->
                <button
                    on:click={toggleNav}
                    class="flex items-center gap-2 font-normal font-inter text-xs text-card"
                    >Share <img src="/icons/whiteshare.svg" alt="" />
                </button>
                {#if open}
                    <!-- svelte-ignore a11y-no-static-element-interactions -->
                    <div
                        ...
                        class="bg-black flex flex-row items-baseline gap-1 p-3 rounded-lg absolute -top-3 left-[58px] cursor-pointer"
                    >
                        ...
                        <a
                            href="mailto:?subject=FrozenPlain blog&body=Check out this blog {$page.url}"
                        >
                            <svg
                                width="19"
                                height="13"
            ...
            <img src={doc.featured_image} alt="" class="w-4/5 mx-auto" />
            <div class="text-card pt-16">
                <svelte:component this={component} />
            </div>
            <Reply
                on:formsubmit={addComment}
                {comments}
                on:updateLikes={updateLikes}
